...
new g_infectnade[33] // has infection nade
new g_unlimited_clip[33] // has unlimited_clip
new g_painshockfree[33] // has pain shock free
...
public plugin_natives()
{
	...
	register_native("zp_get_user_painshockfree", "native_get_user_painshockfree", 1)
...
public message_cur_weapon(msg_id, msg_dest, msg_entity)
{
	...
	if(g_zombie[msg_entity] && user_has_weapon(msg_entity, CSW_HEGRENADE) && get_pcvar_num(cvar_extrainfbomb))
	{
		g_infectnade[msg_entity] = true
	}
	else
	{
		g_infectnade[msg_entity] = false
	}
	
	if ((g_survivor[msg_entity] && get_pcvar_num(cvar_survinfammo) > 1) || (g_unlimited_clip[msg_entity]) || (!g_survivor[msg_entity] && get_pcvar_num(cvar_infammo) > 1))
	{
		// Get weapon's id
		static weapon
...
#include "include/zp_newnatives.inl"
