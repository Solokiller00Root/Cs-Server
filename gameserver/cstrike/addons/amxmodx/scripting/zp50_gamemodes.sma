...
#define COUNT_DOWN_TASK 652452
#define HUD_COLOR_RED 255 // Red
#define HUD_COLOR_GREEN 255 // Green
#define HUD_COLOR_BLUE 0 // Blue

new const TAG[] = "Infection starts in"
...
new sounds[][] = {
    "misc/1.wav", "misc/2.wav", "misc/3.wav", "misc/4.wav", "misc/5.wav",
    "misc/6.wav", "misc/7.wav", "misc/8.wav", "misc/9.wav", "misc/10.wav"
};
...
public countDown(){
	...
	set_hudmessage(HUD_COLOR_RED,HUD_COLOR_GREEN,HUD_COLOR_BLUE,HUD_POSITION_X, HUD_POSITION_Y, 0, 0.0, 1.1, 0.0, 0.0, 1 );
	ShowSyncHudMsg(0, gg_msgsync, "%s %d..",TAG, Timer);
    Timer--;

    //emit_sound(0, CHAN_VOICE, sounds[Timer], VOL_NORM, ATTN_NORM, 0, PITCH_NORM);

	client_cmd(0, "spk ^"%s^"", sounds[Timer]); // .wav
    set_task(1.0, "countDown", COUNT_DOWN_TASK);
...
public event_round_start()
{
	...
	set_task(0.2, "countDown", COUNT_DOWN_TASK);
